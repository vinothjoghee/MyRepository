package com.k2js.MavenSeleniumBasic.Test2;
...
public class Test2 {
	...
	public void VerifyHomePage() throws IOException
	{
		...
		cd.get(p.getProperty("config.url"));
		cd.manage().window().maximize();
		String exp_title=p.getProperty("home");
		String act_title=cd.getTitle();
		
		Assert.assertEquals(exp_title, act_title);
		//Assert.assertEquals(exp_title, act_title,"hi");
	...
	@Test
	public void verifySignOf() throws IOException {
		System.setProperty("webdriver.chrome.driver",
				"D:\\SeleniumTrainingByJitendra\\Tools\\chromedriver_win32\\chromedriver.exe");
		ChromeDriver cd = new ChromeDriver();
		...
		WebElement signoff = cd.findElement(By.xpath("//a[text()='SIGN-ON']"));
		signoff.click();
		
		String userNme=p.getProperty("Signof.un");
		String pw=p.getProperty("Signof.pw");
		
		WebElement un=cd.findElementByName("userName");
		un.sendKeys(userNme);
		
		WebElement pwd=cd.findElementByName("password");
		pwd.sendKeys(pw);
		
		WebElement submit = cd.findElement(By.name("login"));
		submit.click();	
